...
#gprod = pd.Panel(
#    [[[m.variables.gprod[s, g, t].x for g in m.data.generators] for t in m.data.mytaus] for s in m.data.scenarios],
#    items=m.data.scenarios, major_axis=m.data.mytaus, minor_axis=m.data.generators)
#loadshed = pd.DataFrame(
#    [[sum(m.variables.loadshed[s, n, t].x for n in m.data.nodeorder) for s in m.data.scenarios] for t in m.data.mytaus],
#    index=m.data.mytaus, columns=m.data.scenarios)
## Load served is upper bound on load, less the amount shed
#loadserved = pd.DataFrame(
#    [[sum(m.variables.loadshed[s, n, t].ub - m.variables.loadshed[s, n, t].x for n in m.data.nodeorder) for s in m.data.scenarios] for t in m.data.mytaus],
#    index=m.data.mytaus, columns=m.data.scenarios)
